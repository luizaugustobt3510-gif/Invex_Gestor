...
const SolicitacaoOS = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user?.companyId) return;
    if (!form.equipamento || !form.tipo_servico || !form.descricao) {
    ...
    // Integração com Financeiro: criar conta a pagar
    if (valor > 0) {
      await financeiroService.createEntry({
        company_id: user.companyId,
        user_id: userId,
        tipo: 'despesa',
        descricao: `OS Manutenção: ${form.equipamento} - ${form.tipo_servico}`,
        valor,
        data: new Date().toISOString().split('T')[0],
        status: 'pendente',
        origem: 'manutencao_os',
  ...
  const handleStatusChange = async (orderId: string, newStatus: string) => {
    await supabase.from('maintenance_service_orders').update({ status: newStatus }).eq('id', orderId);
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input value={form.equipamento} onChange={e => setForm(p => ({ ...p, equipamento: e.target.value }))} placeholder="Equipamento" />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <Textarea value={form.descricao} onChange={e => setForm(p => ({ ...p, descricao: e.target.value }))} placeholder="Descreva o problema..." />
                  </div>
                  ...
                  <div>
                    ...
                    <Textarea value={form.observacoes} onChange={e => setForm(p => ({ ...p, observacoes: e.target.value }))} placeholder="Informações adicionais..." />
                  </div>
                      ...
                      {loading ? (
                        ...
                        orders.map(o => (
                          <TableRow key={o.id}>
                            ...
                            <TableCell className="max-w-[200px] truncate" title={o.descricao}>{o.descricao}</TableCell>
                            <TableCell>
                              <Badge className={`${prioridadeColor[o.prioridade] || ''} hover:opacity-80`}>{o.prioridade}</Badge>
                            </TableCell>
                            ...
                            <TableCell>{o.solicitante_nome}</TableCell>
                            <TableCell>
                              {canManage ? (
