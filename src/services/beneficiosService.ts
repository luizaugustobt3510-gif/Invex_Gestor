      ...
      if (companyCost > 0) {
        const { data: expense } = await supabase
          ...
          .insert({
            ...
            user_id: userId,
            tipo: 'despesa',
            descricao: `Benefício ${benefit.name} - ${link.employees?.nome ?? 'Funcionário'} (${competencia})`,
            valor: companyCost,
            data: dataLancamento,
      ...
      if (employeeCost > 0) {
        const { data: discount } = await supabase
          ...
          .insert({
            ...
            user_id: userId,
            tipo: 'receita',
            descricao: `Desconto ${benefit.name} - ${link.employees?.nome ?? 'Funcionário'} (${competencia})`,
            valor: employeeCost,
            data: dataLancamento,
            data_vencimento: dataLancamento,
            status: 'pendente',
